...
def encrypt(key, passphrase):
    ...
    if isinstance(key, str):
        if key[0] == "5":
            ...
            # To keep the last 0x01 byte wiped by wif_decode
            key = wif_decode(key) + 0x01.to_bytes(1, 'big')
        else:
            raise ValueError(
                "Private key passed as first argument is not a valid WIF-encoded privkey")
...
def decrypt(key, passphrase):
    ...
    # Version is 2 bytes length, cf encrypt()
    dec = base58check_decode(key, n=2)
    flag = dec[:1]
    addresshash = dec[1:5]
    ...
    privkey = int.from_bytes(
        decryptedhalf1+decryptedhalf2, 'big') ^ int.from_bytes(derivedhalf1, 'big')
    ...
    if flag == 0xe0.to_bytes(1, 'big'):  # Compressed
        ...
        raise ValueError(
            "Key passed as first parameter was malformed. Couldn't read flag byte.")
